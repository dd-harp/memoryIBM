test_that("ragged integer variable functions as expected", {
  library(individual)

  size <- 10
  vals <- replicate(n = size, expr = rbinom(n = rpois(n = 1,lambda = 10), size = 100, prob = 0.6), simplify = FALSE)
  vals_len <- vapply(X = vals, FUN = length, FUN.VALUE = integer(1), USE.NAMES = FALSE)

  var <- IntegerRaggedVariable$new(vals)

  expect_equal(memoryIBM:::integer_ragged_variable_get_size(var$.variable), size)
...
test_that("ragged integer variable errors as expected", {
  library(individual)
